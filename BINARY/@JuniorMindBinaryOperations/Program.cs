...
namespace BinaryOperations
{
    class Program
    {
        ...
        static void ExecuteBinaryFactorial()
        {
            if (!ReadBinaryNumber(out byte[] firstBinaryNumber))
            {
                return;
            ...
            byte[] result = firstBinaryNumber;
            int length = ConvertToDecimal(result);
            for (int i = 1; i < length; i++)
            {
                result = BinaryMultiply(result, ConvertToBinary(i));
            }

            Console.WriteLine(BinaryNumberToString(result));
        }
